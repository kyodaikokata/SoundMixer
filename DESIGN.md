...
> 对齐当前实现 **0.2.3.0**。早期 MVP 草案（FMOD 缓冲、500% 上限等）已过时；以本文档、游戏内「更新日志」与 [KNOWN_ISSUES.md](./KNOWN_ISSUES.md) 为准。
...
- **安全与黑名单**：可选安全模式、官方/用户黑名单、行动守卫（Hook guards）
...
| 机制 | 行为 |
|------|------|
| **安全模式关（默认）** | 骑乘期混音照常；坐骑循环路径透传播放 hook；官方黑名单屏蔽高风险路径 |
| **安全模式开** | 骑乘及 grace 期间挂起全部 hook；跳过活跃链表扫描 |
| **行动守卫** | 骑乘禁用 PlaySystem/Clip/Movie；PlaySound 默认不加载 |

详见 `KNOWN_ISSUES.md` §3 与游戏内 changelog。
